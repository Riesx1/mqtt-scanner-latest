...
def publish_insecure():
    ...
    client.publish("sensors/faris/multi_insecure", json.dumps(data), qos=0, retain=True)
    print(f"Published to insecure broker: {data}")
    client.disconnect()

def publish_secure():
    client = mqtt.Client("test-publisher-secure")
    client.username_pw_set("faris@gmail.com", "faris123")
    ...
    client.publish("sensors/faris/multi_secure", json.dumps(data), qos=0, retain=True)
